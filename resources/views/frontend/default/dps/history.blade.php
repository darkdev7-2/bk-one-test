            ...
            <div class="site-card">
                <div class="site-card-header">
                    ...
                    <div class="card-header-links">
                        <a href="{{ route('user.dps.index') }}" class="card-header-link"><i
                                data-lucide="archive"></i>{{ __('DPS Plan List') }}</a>
                    </div>
                </div>
                        ...
                        <div class="table-filter">
                            <div class="filter">
                                <div class="single-f-box">
                                    ...
                                    <input class="search" type="text" name="dps_id" value="{{ request('dps_id') }}"
                                        autocomplete="off" />
                                </div>
                                <div class="single-f-box">
                            ...
                            @foreach ($dpses as $dps)
                                <div class="site-table-list">
                                    <div class="site-table-col">
                                        <div class="description">
                                            <div class="content">
                                                ...
                                                <div class="date">{{ date('d M Y h:i A', strtotime($dps->created_at)) }}
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="site-table-col">
                                        <div class="description">
                                            <div class="content">
                                                ...
                                                <div class="date">{{ __('Every') }} {{ $dps->plan->interval }}
                                                    {{ __('Days') }}</div>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="site-table-col">
                                        <div class="description">
                                            <div class="content">
                                                ...
                                                <div class="date">{{ __('Given') }}: {{ $dps->given_installment }}
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    @php
                                        ...
                                        $currentDate = now();
                                        $cancel_fee =
                                            $dps->plan->cancel_fee_type == 'percentage'
                                                ? ($dps->plan->cancel_fee / 100) * $dps->plan->per_installment
                                                : $dps->plan->cancel_fee;
                                    @endphp
                                    <div class="site-table-col">
                                        <div class="action">
                                            <a href="{{ route('user.dps.details', $dps->dps_id) }}"
                                                class="icon-btn me-2"><i data-lucide="eye"></i>{{ __('Details') }}</a>
                                            @if (
                                                ($dps->plan->can_cancel && $dps->status->value == App\Enums\DpsStatus::Running->value) ||
                                                    $dps->status->value == App\Enums\DpsStatus::Due->value)
                                                @if ($dps->plan->cancel_type == 'fixed' && $currentDate->diffInDays($creationDate) <= $cancellationDays)
                                                    <span data-bs-toggle="modal" data-bs-target="#cancelDPS">
                                                        <button class="circle-btn red-btn cancelBtn"
                                                            data-id="{{ $dps->dps_id }}"
                                                            data-fee="{{ $cancel_fee }} {{ $currency }}"
                                                            data-bs-custom-class="custom-tooltip" data-bs-toggle="tooltip"
                                                            data-bs-placement="top" data-bs-title="{{ __('Cancel DPS') }}">
                                                            <i data-lucide="x"></i>
                                                        </button>
                                                    </span>
