...
<style>
    .site-input-groups .box-textarea {
    width: 100%;
    border-radius: 6px;
                        ...
                        <form action="{{route('admin.user.update',$user->id)}}" enctype="multipart/form-data" method="post">
                            ...
                            <div class="row">
                                @php
                                    ...
                                    $user->custom_fields_data  = is_null($user->custom_fields_data) ? [] : $user->custom_fields_data;
                                @endphp
                                @if ($registerCustomFields)
                                    @foreach ($registerCustomFields as $key => $field)
                                        <div class="col-xl-6 col-md-12">
                                            <div class="site-input-groups">
                                                <label class="box-input-label" for="">{{ $field['name'] }}</label>
                                                @if($field['type'] == 'textarea')
                                                    <textarea name="custom_fields_data[{{ $field['name'] }}]" class="box-textarea">{{ $user->custom_fields_data[$field['name']] ?? '' }}</textarea>
                                                ...
                                                <div class="wrap-custom-file">
                                                        <input type="file" name="custom_fields_data[{{ $field['name'] }}]" id="{{ $key }}" accept=".gif, .jpg, .png" />
                                                        <label for="{{ $key }}" @if($oldFile) class="file-ok" style="background-image: url({{ asset($filePath) }})" @endif>
                                                            <img class="upload-icon" src="{{ asset('front/images/icons/upload.svg') }}" alt="" />
                                                            <span>{{ $field['name'] }}</span>
                                                        </label>
                                                    </div>
                                                @else
                                                    <input type="text" name="custom_fields_data[{{ $field['name'] }}]" class="box-input" value="{{ $user->custom_fields_data[$field['name']] ?? '' }}" />
                                                @endif
                                            </div>
                                        </div>
                                    @endforeach
